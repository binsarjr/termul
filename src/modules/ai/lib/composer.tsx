...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  const attachFileByPath = async (path: string) => {
    try {
      const result = await fsBridge.readFile(path);
      if (result.kind !== "text") {
  ...
  const submit = () => {
    ...
    if (!sessionId) return;
    // The chat runtime (AI SDK + transport + tools) is a heavy lazy chunk;
    // load it on first send instead of pulling it into startup.
    void import("../store/chatRuntime").then(({ getOrCreateChat }) => {
  ...
  const stop = () => {
    if (!sessionId) return;
    void import("../store/chatRuntime").then(({ getOrCreateChat }) =>
      getOrCreateChat(sessionId).stop(),
    );
  };
