...
const OWNED_MARKERS: [&str; 2] = ["notify;Termul;", "termul;notify"];

// Gated on TERMUL_TERMINAL; no-op outside Termul. Returns the sequence via
// `terminalSequence` because hooks lost /dev/tty access in v2.1.139.
fn hook_cmd(event: &str) -> String {
    format!(
        r#"[ -n "$TERMUL_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;Termul;{event}\\u0007"}}' || true"#
    )
}
...
pub fn agent_enable_claude_hooks() -> Result<(), String> {
    ...
    let tmp = path.with_extension("json.termul-tmp");
...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;Termul;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;Termul;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;Termul;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;Termul;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$TERMUL_TERMINAL\" ] && printf '\\033]777;termul;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
