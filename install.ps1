# Its Just Terminal installer for Windows.
#
...
# (More info -> Run anyway). Overridable via env: IJT_REPO, IJT_VERSION.

#Requires -Version 5
$ErrorActionPreference = 'Stop'

$Repo    = if ($env:IJT_REPO)    { $env:IJT_REPO }    else { 'binsarjr/its-just-terminal' }
$Version = if ($env:IJT_VERSION) { $env:IJT_VERSION } else { 'latest' }
...
$headers = @{ 'User-Agent' = 'ijt-installer'; 'Accept' = 'application/vnd.github+json' }
if ($env:GITHUB_TOKEN) { $headers['Authorization'] = "Bearer $($env:GITHUB_TOKEN)" }

Info "Its Just Terminal installer ($Repo @ $Version)"
...
Info "Installed Its Just Terminal $($release.tag_name)"
